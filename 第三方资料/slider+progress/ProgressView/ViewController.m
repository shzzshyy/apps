...
//    [self.view addSubview:_ballProgress];
    ...
    _circleProgress = [[ZXYCircleProgress alloc] initWithFrame:CGRectMake(100, 100, 30, 30) progress:0];
    _circleProgress.progressWidth = 3;
    _circleProgress.bottomColor = [UIColor redColor];
    _circleProgress.topColor = [UIColor blueColor];
...
//    _sectorProgress.progress = (sender.value - sender.minimumValue) / (sender.maximumValue - sender.minimumValue);
    CGFloat xx = (sender.value - sender.minimumValue) / (sender.maximumValue - sender.minimumValue);
    NSLog(@"%.2f",xx);
