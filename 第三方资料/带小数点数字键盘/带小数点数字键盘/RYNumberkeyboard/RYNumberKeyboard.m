...
@interface RYNumberKeyboard ()<UITextFieldDelegate>
...
#pragma mark-这里限制只能是.后两位
    int myIndex = 0;
    
    NSMutableArray *arr = [NSMutableArray array];
    for (int i=1; i<=self.textFiled.text .length; i++) {
        [arr addObject:[self.textFiled.text  substringWithRange:NSMakeRange(i-1, 1)]];
    ...
    if(myIndex!=0){
        ...
        if (self.textFiled.text .length-myIndex>2) {
